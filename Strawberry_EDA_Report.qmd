...
From this Plot we can see that California provide the most operation with sales, and the trend in time scale is decreasing for most states.
...
plot2_data <- plot2_data |>
    filter((as.numeric(Value) > 50000)&(Metric == "$"))
...
plot2_data |>
    ...
    labs(
        ...
        subtitle = "With Value larger than 50,000$",
        y = "Values in $"
    ) +
    geom_smooth(aes(x = Year, y = Value ), method = "lm", se = F, size = 0.5)+
    theme_light() + 
...
From this Plot we can see that California provide the most sales profit in \$, and the trend in time scale is increasing for most states.
